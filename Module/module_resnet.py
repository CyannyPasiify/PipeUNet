...
核心功能：
    1. 支持 3D ResNet、ConvNeXt3D HFResNet3DCustom 等模型的训练和评估
...
# 导入模型
from Network.resnet_convnext_3d import (
    ResNet3D,
    ConvNeXt3D,
    BasicBlock3D,
    Bottleneck3D
)
from Network.hf_resnet3d import HFResNet3DCustom
...
class ResNetLightningModule(L.LightningModule):
    """
    ResNet Lightning 模块类
    支持各种 3D ResNet 变体和 ConvNeXt3D 模型
    ...
    def __init__(self,
        ...
        Args:
            model_type: 模型类型，可选值: 'resnet3d', 'convnext3d', 'hf_resnet3d'
            model_config: 模型配置参数
            learning_rate: 初始学习率
            weight_decay: 权重衰减系数
    ...
    def _create_model(self, model_type: str, model_config: Dict[str, Any]) -> Union[ResNet3D, ConvNeXt3D]:
        """
        创建模型实例
        ...
        if model_type == 'resnet3d':
            ...
            return ResNet3D(**model_config)
        elif model_type == 'convnext3d':
            # ConvNeXt3D 模型配置
            return ConvNeXt3D(**model_config)
        elif model_type == 'hf_resnet3d':
            # HuggingFace pretrained ResNet3D 模型配置
            return HFResNet3DCustom(**model_config)
        else:
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        # 合并通道维度
        x = torch.cat([
            pre_masked_img if self.hparams.roi_only else pre_img,
            pre_masked_img,
            post_masked_img if self.hparams.roi_only else post_img,
            post_masked_img
        ], dim=1)
...
def create_resnet_module(model_type: str = 'resnet3d',
                         ...
                         eta_min: float = 1e-6,
                         loss_type: str = 'cross_entropy',
                         loss_config: Optional[Dict[str, Any]] = None) -> ResNetLightningModule:
    """
...
if __name__ == "__main__":
    # 示例用法
    print("创建 ResNet-18 3D Lightning 模块示例（兼容 EsophagusDataModule）")

    # 创建 ResNet-18 3D 模型配置
    resnet_config = {
        'block': 'basic',
        'layers': [2, 2, 2, 2],
        'in_channels': 4,  # 兼容 EsophagusDataModule，使用 4 通道（pre_img, pre_masked_img 和 post_img, post_masked_img）
        'num_classes': 2
    }
