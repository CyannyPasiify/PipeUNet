# -*- coding: utf-8 -*-
"""
 -r/--root_dir指定根目录，根目录下包含若干层级的子目录，最低层为site中心目录，中心目录下包含phase阶段目录，即治疗前pre和治疗后post目录，阶段目录中包含{site}_{pid}_{phase}样本目录，每个中包含形如{site}_{pid}_{phase}_info.yaml的元信息文件，{site}_{pid}_{phase}_volume.nii.gz的图像文件，{site}_{pid}_{phase}_mask.nii.gz的蒙版文件，{site}_{pid}_{phase}_mask_{label_index}_{label_name}.nii.gz的二值蒙版文件。
 接收-o/--output_manifest_file 
...
"""
Dataset Manifest Generation Script for ESO-2025 Dataset

This script generates a comprehensive manifest of the ESO-2025 dataset, including volume and mask file paths,
...
def process_sample_dir(sample_dir: Path, root_path: Path, loader: LoadImage, verbose: bool = False) -> Optional[
    ...
    # Get site, phase, and pid from the directory structure
    ...
    phase: str = sample_dir.parent.name.lower() if sample_dir.parent.name.lower() in ['pre', 'post'] else ''
    ...
    try:
        ...
        yaml_fields = [
            ...
            'cN Stage', 
            'cTNM', 
            'Neotime - Date of First', 
            'Neoadjuvant Therapy',
            'Surgicaltime - Date of Surgery', 
            'Hypertension', 
            'Diabetes Mellitus',
            'Alcohol Consumption', 
            'Smoking Status', 
            ...
            'R0', 
            'RRECIST',
            'Differentiation Grade (0=Gx 1=G1 2=G2 3=G3)', 
            ...
            'Recurrence Pattern',
            'Recurrence: Cut-off Date 2025-07-28', 
            'Death: Cut-off Date 2025-07-28',
            'Time to Recurrence', 
...
def main() -> None:
    ...
    required_columns = [
        'ID', 'site', 'phase', 'pid', 'valid_labels', 'info', 'volume', 'mask'
    ]
    ...
    # Metadata from YAML
    yaml_fields = [
        'Sex', 'Age', 'Chemotherapeutic Drugs', 'Immunotherapeutic Drugs', 'Location',
        'cT Stage', 'cN Stage', 'cTNM', 'Neotime - Date of First', 'Neoadjuvant Therapy',
        'Surgicaltime - Date of Surgery', 'Hypertension', 'Diabetes Mellitus',
        'Alcohol Consumption', 'Smoking Status', 'Neoadjuvant Cycles', 'Height',
        'Weight', 'BMI', 'Operation Duration', 'Minimally Invasive Surgery',
        'Surgical Type (1=Three-incision 2=Left Thoracotomy Approach 3=Two-incision)',
        'Thoracic Duct Ligation', 'R0', 'RRECIST',
        'Differentiation Grade (0=Gx 1=G1 2=G2 3=G3)', 'Tumor Regression Grade (TRG)',
        'Major Pathological Response (MPR)', 'pT Stage', 'pN Stage',
        'Pathological Complete Response (pCR)', 'Lymph Node Metastasis',
    ...
    all_columns = (required_columns + mask_columns + existence_columns +
                   image_properties + yaml_fields)
