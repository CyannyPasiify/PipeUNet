...
class UnionWrapperMaintainer(WrapperTypeMaintainer):
    """Union type wrapper maintainer"""
    
    def __init__(self, maintainers: List[WrapperTypeMaintainer]):
        """Initialize
        
        Args:
            maintainers: List of type maintainers
        """
    ...
    def get_default_value(self) -> Any:
        ...
        Return the default value of the first maintainer
        """
        if self.maintainers:
            return self.maintainers[0].get_default_value()
        return None
    
    def is_compatible(self, value: Any) -> bool:
        """Check if value is compatible with the type
        
        Compatible if any of the maintainers accepts the value
        """
    ...
    def get_expected_type_name(self) -> str:
        ...
        type_names = [maintainer.get_expected_type_name() for maintainer in self.maintainers]
        return f"Union[{', '.join(type_names)}]"
    
    def create_edit_control(self, parent, value, on_change):
        """Create edit control for Union value
        ...
        # Get type names and sort them in the specified order
        type_order = ["None", "bool", "int", "float", "str"]
        
        # Create a list of (type_name, maintainer) pairs
        type_maintainer_pairs = [(maintainer.get_expected_type_name(), maintainer) for maintainer in self.maintainers]
        
        # Sort the pairs according to the specified order
        type_maintainer_pairs.sort(key=lambda x: type_order.index(x[0]) if x[0] in type_order else len(type_order))
        
        # Extract the sorted type names and maintainers
        type_names, sorted_maintainers = zip(*type_maintainer_pairs)
        type_names = list(type_names)
        sorted_maintainers = list(sorted_maintainers)
        
        type_var.set(type_names[0])  # Default to first type
        
        # Find the maintainer that is compatible with the current value
        current_maintainer_index = 0
        for i, maintainer in enumerate(sorted_maintainers):
            if maintainer.is_compatible(value):
                current_maintainer_index = i
                type_var.set(type_names[i])
                break
        ...
        def update_edit_control():
            ...
            selected_index = type_names.index(selected_type)
            selected_maintainer = sorted_maintainers[selected_index]
    ...
    def render_control(self, parent, attribute_name, value, on_change):
        """Render control for editing Union attribute"""
        frame = ttk.Frame(parent)
