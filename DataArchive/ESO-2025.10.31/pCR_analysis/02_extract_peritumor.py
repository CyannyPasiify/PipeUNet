...
by specified distances and subtracting the original foreground region.
...
Examples:
  ...
  %(prog)s --root_dir /path/to/root --peri_expand_range 3.0 5.0 7.0 --mask_type '' 'peritumor'
        """)
...
def extract_peritumor_region(mask_path: Path, expand_distance: float) -> np.ndarray:
    """
    ...
    dilated_mask: np.ndarray = binary_dilation(binary_mask, structure=structuring_element)
    
    # Subtract the original mask to get peritumor region
    peritumor_mask: np.ndarray = dilated_mask - binary_mask
...
def process_sample_dir(sample_dir: Path, peri_expand_range: List[float], mask_type: str) -> int:
    """
    ...
    if pre_mask:
        for distance in peri_expand_range:
            try:
                ...
                meta_data: Dict
                original_data, meta_data = load_image(str(pre_mask))
                
                # Extract peritumor region
                peritumor_mask: np.ndarray = extract_peritumor_region(pre_mask, distance)
    ...
    if post_mask:
        for distance in peri_expand_range:
            try:
                ...
                original_data: np.ndarray
                meta_data: Dict
                original_data, meta_data = load_image(str(post_mask))
                
                # Extract peritumor region
                peritumor_mask: np.ndarray = extract_peritumor_region(post_mask, distance)
    ...
    with tqdm(sample_dirs, desc="Processing samples", unit="sample") as pbar:
        for sample_dir in pbar:
            ...
            for mask_type in args.mask_type:
                masks_created: int = process_sample_dir(sample_dir, args.peri_expand_range, mask_type)
