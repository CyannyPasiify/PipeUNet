...
Usage Examples:
    python 02_amos_mri_confirm_pairs.py -r /path/to/AMOS22 -o /path/to/AMOS-MRI/grouped -am /path/to/archive_manifest.xlsx
    python 02_amos_mri_confirm_pairs.py --root_dir /path/to/AMOS22 --output_dir /path/to/AMOS-MRI/grouped --archive_manifest /path/to/archive_manifest.xlsx
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Confirm and reorganize image-mask pairs for AMOS22 MRI dataset',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s -r /path/to/AMOS22 -o /path/to/AMOS-MRI/grouped -am /path/to/archive_manifest.xlsx
  %(prog)s --root_dir /path/to/AMOS22 --output_dir /path/to/AMOS-MRI/grouped --archive_manifest /path/to/archive_manifest.xlsx
        """
    )
...
def load_archive_manifest(manifest_path):
    ...
    valid_manufacturers = {'Ingenia', 'Prisma', 'SIGNA HDe', 'Achieva'}
...
def process_pairs(root_dir, output_dir, file_info_mapping, metadata_mapping):
    ...
    for (subset, scanner), pairs in tqdm(pairs_by_group.items(), desc='Processing groups'):
        output_subdir = output_path / subset / scanner
        output_subdir.mkdir(parents=True, exist_ok=True)

        for seq_id, image_path, mask_path, _ in tqdm(pairs, desc=f'  {subset}/{scanner}', leave=False):
