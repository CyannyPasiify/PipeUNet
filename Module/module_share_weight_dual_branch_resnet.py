...
# -*- coding: utf-8 -*-
"""
DualResNet Lightning 模块

功能概述：
    提供基于 PyTorch Lightning 的 ResNet 模块实现
    集成模型、优化器、损失函数和学习率调度器
    兼容 EsophagusDataModule 数据格式
    支持多种性能指标的记录和可视化

核心功能：
    1. 支持共享权重双分支 3D ResNet、ConvNeXt3D 等模型的训练和评估
...
# 导入模型
from Network.share_weight_dual_branch_resnet import (
    DualResNet3D,
    DualConvNeXt3D,
    BasicBlock3D,
    Bottleneck3D
...
class DualResNetLightningModule(L.LightningModule):
    """
    共享权重双分支ResNet Lightning 模块类
    支持各种共享权重双分支 3D ResNet 变体和 ConvNeXt3D 模型
    ...
    def __init__(self,
        ...
        """
        初始化 ResNet Lightning 模块
        
        Args:
            model_type: 模型类型，可选值: 'resnet3d', 'convnext3d'
            model_config: 模型配置参数
            learning_rate: 初始学习率
            weight_decay: 权重衰减系数
    ...
    def _create_model(self, model_type: str, model_config: Dict[str, Any]) -> Union[DualResNet3D, DualConvNeXt3D]:
        """
        创建模型实例
        ...
        if model_type == 'resnet3d':
            # DualResNet3D 模型配置
            if isinstance(model_config['block'], str):
                model_config['block'] = BasicBlock3D if model_config['block'] == 'basic' else Bottleneck3D
            return DualResNet3D(**model_config)
        elif model_type == 'convnext3d':
            # DualConvNeXt3D 模型配置
            return DualConvNeXt3D(**model_config)
        else:
    ...
    def validation_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> Dict[str, torch.Tensor]:
        ...
        # 合并通道维度
        x = torch.cat([pre_img, pre_masked_img, post_img, post_masked_img], dim=1)
...
def create_resnet_module(model_type: str = 'resnet3d',
                         ...
                         eta_min: float = 1e-6,
                         loss_type: str = 'cross_entropy',
                         loss_config: Optional[Dict[str, Any]] = None) -> DualResNetLightningModule:
    """
    创建 ResNet Lightning 模块的工厂函数
    ...
    Returns:
        ResNetLightningModule 实例
    """
    # 确保模型配置中设置了正确的输入通道数（兼容 EsophagusDataModule，通常为 4）
    if model_config is None:
        model_config = {}

    # 指定输入通道数为 4 = 2 + 2（pre_img, pre_masked_img 和 post_img, post_masked_img 各 1 通道）
    model_config['branch_in_channels'] = 2

    return DualResNetLightningModule(
...
if __name__ == "__main__":
    # 示例用法
    print("创建 DualResNet-18 3D Lightning 模块示例（兼容 EsophagusDataModule）")

    # 创建 ResNet-18 3D 模型配置
    resnet_config = {
        'block': 'basic',
        'layers': [2, 2, 2, 2],
        'branch_in_channels': 2,  # 兼容 EsophagusDataModule，使用 4 = 2 + 2 通道（pre_img, pre_masked_img 和 post_img, post_masked_img）
        'num_classes': 2
    }

    # 创建 Lightning 模块
    module = create_resnet_module(
        model_type='resnet3d',
