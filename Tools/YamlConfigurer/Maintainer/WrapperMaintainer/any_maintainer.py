...
class AnyMaintainer(WrapperMaintainer):
    ...
    def __init__(
        ...
        if self.is_type_compatible():
            ...
            for t in available_types:
                try:
                    mt_cls: Type[BaseMaintainer] = MaintainerFactory.get_maintainer_cls_supported_type(t)
                    self.maintainer_cls.append(mt_cls)
                    self.map_maintainer_cls[
                        MaintainerFactory.get_simplest_type_name(t)
                    ] = (t, mt_cls)
                except Exception as e:
                    if self.logger:
                        assert hasattr(self.logger, "log_message"), \
                            f"Logger must implement 'log_message(message, level)' function"
                        self.logger.log_message(
                            f"{self.__class__.__name__} encounted type exception for attribute '{attribute_name}' "
    ...
    def is_type_compatible(self) -> bool:
        ...
        return self.attribute_type is Any
    ...
    def is_type_compatible_static(target_type: Type) -> bool:
        return simplify_type(target_type) is Any
