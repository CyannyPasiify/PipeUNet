...
class GradCAM:
    ...
    def compute_cam(self, input_tensor: Dict[str, torch.Tensor], target_class: Optional[int] = None):
        ...
        radiomics_features = input_tensor['radiomics']

        # 前向传播
        self.model.zero_grad()
        outputs = self.model(x, radiomics_features)
...
def main():
    ...
    parser.add_argument("-p", "--prefix", type=str, choices=['pre', 'post'], default=None, help='使用蒙版掩蔽CAM图')
    parser.add_argument("-rd", "--root_dir", type=str, required=True, help='数据集索引根目录')
    parser.add_argument("-nr", "--n_radiomics", type=int, required=True, help='影像组学特征数量')
    parser.add_argument("-mf", "--manifest_file", type=str, required=True, help='清单文件路径')
    parser.add_argument("-id", "--pid", type=str, required=True, help='目标样本编号')
    ...
    print("加载模型...")
    # 加载模型 使用 IDDFENetLightningModule 从检查点加载
    from Module.module_iddfenet import IDDFENetLightningModule

    # 从检查点加载整个模型
    model = IDDFENetLightningModule.load_from_checkpoint(args.checkpoint, map_location=torch.device('cpu'))
    ...
    print("加载输入数据...")
    manifest: pd.DataFrame = pd.read_excel(args.manifest_file, dtype={'pid': str})
    record: pd.Series = manifest[manifest['pid'] == args.pid].iloc[0]
    ...
    for key in ['pre_img', 'pre_mask']:
        if pd.notna(record[key]):
            full_path: str = os.path.join(args.root_dir, record[key].replace('\\', '/'))
    ...
    # 提取影像组学特征
    radiomics_columns: List[str] = [col for col in manifest.columns if col not in
                                    ['pid', 'pCR', 'center', 'pre_img', 'pre_mask', 'post_img', 'post_mask']]
    radiomics_features = record[radiomics_columns].values.astype(float)
    radiomics_tensor: torch.Tensor = torch.tensor(radiomics_features, dtype=torch.float32)
    process_data['radiomics'] = radiomics_tensor
    print("数据加载完成")
