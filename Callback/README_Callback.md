...
- `transform_configurer`：**关键代码**。定义了`ConfigCallbackBase`**回调配置包装器**基类，以及来自Lightning的8个常用预设回调的配置包装器。
...
可以使用`callback_configurer`的主例程和本项目提供的示例样本（样本在`Samples`目录中）进行快速测试或调试以观察执行细节。命令行参数设置`--callback all`以测试全部回调包装器，或指定要测试的回调包装器名称（请去除共同前缀`ConfigCallback`）。请从项目根启动主例程以确保相对路径的正确性。
