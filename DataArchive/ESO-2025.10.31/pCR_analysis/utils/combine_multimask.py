...
Parameters:
    ...
    -f, --inffix: Infix for output mask filename (default: 'multi')
    --skip_existing: Skip processing if output mask file already exists

Usage Examples:
    python combine_multimask.py -r /path/to/root
    python combine_multimask.py --root_dir /path/to/root --mask_type '' 'peritumor_3.0mm' --phase 'pre' --reindex 1 2 --inffix 'combined'
...
Examples:
  ...
  %(prog)s --root_dir /path/to/root --mask_type '' 'peritumor_3.0mm' --phase 'pre' --reindex 1 2 --inffix 'combined'
...
def combine_masks(sample_dir: Path, phases: List[str], mask_types: List[str], label_indices: List[int],
                  inffix: str, skip_existing: bool = False) -> int:
    """
    ...
    Args:
        ...
        inffix: Infix for output mask filename
        skip_existing: Skip processing if output mask file already exists
    ...
    for phase in phases:
        try:
            ...
            if inffix:
                output_filename: str = f"{base_name}_{phase}_{inffix}_mask.nii.gz"
            else:
...
def main() -> None:
    ...
    print(f"Using output infix: '{args.inffix}'")
