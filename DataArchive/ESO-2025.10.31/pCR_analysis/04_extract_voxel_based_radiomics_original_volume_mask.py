...
# -*- coding: utf-8 -*-
"""
接收-r/--root_dir指定根目录，根目录下包含{site}/{site}_{pid}样本目录。对每个样本，加载其图像文件{site}_{pid}_(pre|post)_volume.nii.gz，使用pyradiomics按照全图作为ROI的模式提取voxel-based特征图。-c/--radiomics_config指定运行pyradiomics的配置文件。提取结果保存在样本目录下的新目录{site}_{pid}_(pre|post)_voxel_radiomics中，文件名为{site}_{pid}_(pre|post)_{对应的radiomic feature name}.nii.gz，radiomic feature name例如origina_firstorder_Entropy，其meta保持与图像一致。 
 此外帮我创建一个radiomics_voxel_based_config.yaml，使用original图像类型，包含以下特征： 
 firstorder_Entropy, firstorder_MeanAbsoluteDeviation, firstorder_Median 
 glcm_DifferenceAverage, glcm_DifferenceEntropy, glcm_DifferenceVaria, glcm_lmc1, glcm_lmc2, glcm_lnverseVariance, glcm_JointEnergy, glcm_JointEntropy, glcm_SumEntropy 
...
using pyradiomics, with the entire image as the ROI. Features are saved as separate NIfTI files.
...
Usage Examples:
    python 03_extract_voxel_based_radiomics.py -r /path/to/root -c radiomics_voxel_based_config.yaml
    python 03_extract_voxel_based_radiomics.py --root_dir /path/to/root --radiomics_config config.yaml
"""
...
def generate_expected_filenames(base_name: str, phase: str, output_dir: Path, radiomics_config: Dict[str, Any]) -> List[Path]:
    """
...
def parse_radiomics_config(config_path: Path) -> Dict[str, List[str]]:
    """
    Parse the radiomics YAML configuration file and extract image types, feature classes, and features.
    
    Args:
        config_path: Path to the radiomics YAML configuration file
        
    Returns:
        Dict[str, List[str]]: Dictionary containing image types, feature classes, and features.
            Format: {'image_types': [...], 'feature_classes': [...], 'features': {'class1': [...], 'class2': [...]}}
    """
...
def load_existing_roi_mask(mask_path: Path) -> sitk.Image:
    """
    Load an existing ROI mask from file.
    
    Args:
        mask_path: Path to the input mask NIfTI file
        
    Returns:
        sitk.Image: SimpleITK image object representing the loaded ROI mask
    """
    # Load mask directly with SimpleITK
    mask_sitk: sitk.Image = sitk.ReadImage(str(mask_path))

    # Ensure mask is binary (0/1)
    mask_sitk = sitk.Cast(sitk.BinaryThreshold(mask_sitk, 1, 255, 1, 0), sitk.sitkUInt8)
...
def extract_voxel_based_radiomics(image_path: Path, mask_path: Path, config_path: Path) -> Dict[str, np.ndarray]:
    ...
    """
    # Load the original image to get full dimensions
    load_image = LoadImage(image_only=False)
    original_data: np.ndarray
    meta_data: Dict
    original_data, meta_data = load_image(str(image_path))
    full_shape = original_data.shape

    # Load the ROI mask to find bounding box
    ...
    for key, value in result.items():
        if isinstance(value, sitk.Image):
            ...
            # We use kernel 3x3 in pyradiomics, so the bbox will expand 1 for each edge
            full_feature_array[x_min - 1:x_max + 2, y_min - 1:y_max + 2, z_min - 1:z_max + 2] = bb_feature_array
...
def process_sample_dir(sample_dir: Path, config_path: Path, skip_existing: bool) -> int:
    """
    ...
    if pre_volume and pre_mask:
        try:
            ...
            output_dir: Path = sample_dir / f"{base_name}_pre_voxel_radiomics"
            if skip_existing:
            ...
            original_data: np.ndarray
            meta_data: Dict
            original_data, meta_data = load_image(str(pre_volume))
    ...
    if post_volume and post_mask:
        try:
            # Check if output directory already exists and contains files
            output_dir: Path = sample_dir / f"{base_name}_post_voxel_radiomics"
            if skip_existing:
                # Generate all expected filenames based on radiomics config
                expected_files: List[Path] = generate_expected_filenames(base_name, 'post', output_dir, radiomics_config)
            ...
            original_data: np.ndarray
            meta_data: Dict
            original_data, meta_data = load_image(str(post_volume))
...
def main() -> None:
    ...
    print(f"Total voxel-based feature maps created: {total_feature_maps}")
