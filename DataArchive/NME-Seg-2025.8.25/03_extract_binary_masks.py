# -*- coding: utf-8 -*-
"""
 -r/--root_dir指定根目录，根目录下包含若干层级的子目录，其结构为{output_dir}/{site}/{subset}/{seq}_{pid}，{seq}_{pid}为样本目录，其中包含形如{seq}_{pid}_mask.nii.gz的蒙版文件。 
 -e/--label_explanation指定label_map.yaml文件，读取其中的short_form_index_map，这是一个字典，按照{label_index}整型数: "{label_name}"记录了标签值及其解释。对每个样本蒙版文件，使用LoadImage读取其内容，将其中的所有标签值（从0开始）对应的蒙版分离出来，转换成多个01二值蒙版，然后使用SaveImage输出，文件保存在相同二级子目录下，文件名为{seq}_{pid}_mask_{label_index:02d}_{label_name}.nii.gz。在处理途中同时检查mask文件中的实际标签值类别，如果有不在short_form_index_map范围内的，则报告错误但不影响程序继续执行。 
