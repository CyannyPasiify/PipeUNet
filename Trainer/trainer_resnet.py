...
class EsophagusTrainer:
    ...
    def _get_model_checkpoints(self) -> List[ModelCheckpoint]:
        ...
        # 验证指标列表
        val_metrics: List[Tuple[str, str]] = [
            ("val/loss", "min"),
            ("val/acc", "max"),
            ("val/precision", "max"),
            ("val/recall", "max"),
    ...
    def fit(self, model: L.LightningModule, datamodule: L.LightningDataModule) -> Dict[str, Any]:
        """
        训练模型
    ...
    def test(self, model: L.LightningModule, datamodule: L.LightningDataModule, ckpt_path: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        测试模型
    ...
    def predict(self, model: L.LightningModule, datamodule: L.LightningDataModule, ckpt_path: Optional[str] = None) -> List[Any]:
        """
        模型预测
...
def create_esophagus_trainer(
    ...
    wandb_project: str = "EsophagusProject",
    enable_ddp: bool = False
) -> EsophagusTrainer:
    """
    创建食管项目训练器的工厂函数
...
if __name__ == "__main__":
    ...
    # 创建训练器实例
    trainer_instance = create_esophagus_trainer(
        max_epochs=200,
        accelerator="gpu" if torch.cuda.is_available() else "cpu",
        devices=1,
        precision=32,
        experiment_name="ESO-2025.11.8_ResNet18",
        experiment_version="20251117"
    )
    
    # 获取配置好的Trainer
    trainer = trainer_instance.get_trainer()
    
    print(f"\n训练器配置完成:")
    print(f"- 实验名称: {EXPERIMENT_NAME}")
    print(f"- 版本号: {EXPERIMENT_VERSION}")
    print(f"- 最大轮数: 200")
    print(f"- 设备: {trainer_instance.accelerator}")
    print(f"- 日志目录: {trainer_instance.log_dir}")
    print(f"- 已配置的日志记录器: CSVLogger, TensorBoardLogger, WandbLogger")
    print(f"- 已配置的回调: Rich进度条, Rich模型摘要(depth=5), 模型检查点(top-3), 早停, 学习率监控")
    
    # 注意：实际使用时，需要导入模型和数据模块
    # 示例代码（需要根据实际情况调整）：
    """
    # 导入模型和数据模块
    ...
    # 训练模型
    trainer_instance.fit(model=model, datamodule=data_module)
    
    # 测试模型
    test_results = trainer_instance.test(model=model, datamodule=data_module)
    """